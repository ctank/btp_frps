...
class btp_frps_main():
    ...
    def install(self, get):
        ...
        cmd = "cd %s && /bin/bash install.sh download \"%s\"" % (
            pluginPath, release['url'])
        public.M('tasks').add(
            'id, name, type, status, addtime, execstr',
            (None, '安装 [' + taskName + '-' + release['version'] + ']',
             'execshell', '0', time.strftime('%Y-%m-%d %H:%M:%S'), cmd))
        cache.delete('install_task')
        public.writeFile('/tmp/panelTask.pl', 'True')
        public.WriteLog('TYPE_SETUP', 'PLUGIN_ADD',
                        (taskName, release['version']))
    ...
    def upload(self, get):
        ...
        try:
            ...
            if file.filename[-7:] == '.tar.gz' or file.filename[-4:] == '.zip':
                ...
                file.save(filename)
                import panelTask
                panelTask.bt_task()._unzip(filename,
                                           pluginPath + '/temp/import', '',
                                           '/dev/null')
                success, failed = public.ExecShell(
                    'find %s/temp/import -name frps' % pluginPath)
                if success.strip() != '':
    ...
    def save(self, get):
        try:
            ...
            public.WriteFile(pluginPath + '/conf/config.json',
                             get['json'],
                             mode='w+')
            keys = {
                "bindAddr": "bind_addr",
...
WantedBy=multi-user.target''' % (frpsPath, frpsIniPath)
        ...
        if os.path.isdir(
                '/etc/systemd/system') and not os.path.isfile(filename):
    ...
    def __pid(self):
        success, failed = public.ExecShell(
            'ps -ef | grep %s | grep -v grep | awk \'{print $2}\' | head -n 1'
            % frpsPath)
        if success.strip() == '':
            return False
